...
const PartnerDashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const filteredData: UnifiedSolarSummary = {
        ...data,
        providers: filteredProviders,
        totalStations: filteredProviders.reduce((sum, p) => sum + p.stations.total, 0),
        totalDevices: filteredProviders.reduce((sum, p) => sum + p.devices.total, 0),
        totalEnergyToday: filteredProviders.reduce((sum, p) => sum + p.energy.today, 0),
        totalEnergyMonth: filteredProviders.reduce((sum, p) => sum + p.energy.thisMonth, 0),
        totalEnergyYear: filteredProviders.reduce((sum, p) => sum + p.energy.thisYear, 0),
    ...
    return (
      <div style={{ padding: '24px', textAlign: 'center', minHeight: '400px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Space direction="vertical" size="large">
          <Spin size="large" />
          <Text>Loading solar energy data from your assigned providers...</Text>
        </Space>
      </div>
    );
  }
  ...
  if (!solarData || solarData.providers.length === 0) {
    return (
      <div style={{ padding: '24px' }}>
        <Empty
          description={
            <Space direction="vertical">
              <Text strong>No Inverters Assigned</Text>
              <Text type="secondary">
                You don't have any inverters assigned to your account yet.
                Please contact your administrator to assign inverters.
              </Text>
            </Space>
          }
        />
      ...
      <Space direction="vertical" size={24} style={{ width: '100%' }}>
            ...
            <Col xs={24} sm={12} lg={6}>
              <Card hoverable style={{ borderLeft: '4px solid #52c41a', height: '100%' }}>
                ...
                <Text type="secondary" style={{ fontSize: '12px' }}>
                  Across {solarData.providers.length} provider{solarData.providers.length > 1 ? 's' : ''}
                </Text>
              </Card>
            </Col>
        ...
        <Row gutter={[16, 16]}>
          {solarData.providers.map((provider: UnifiedSolarData) => (
            <Col xs={24} lg={solarData.providers.length === 1 ? 24 : solarData.providers.length === 2 ? 12 : 8} key={provider.provider}>
              <Card
                title={
                  <Space>
          ...
          <Row gutter={[16, 16]}>
            <Col xs={24} md={8}>
              <div style={{ background: '#f5f5f5', padding: '16px', borderRadius: '8px' }}>
                ...
                <Text type="secondary" style={{ fontSize: '12px' }}>
                  Across your assigned providers
                </Text>
              </div>
            </Col>
            <Col xs={24} md={8}>
