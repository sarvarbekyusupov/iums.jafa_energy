  ...
  useEffect(() => {
    const verifyToken = async () => {
      ...
      const linkParam =
        ...
        searchParams.get("activation_link") ||
      ...
      try {
        ...
        if (response.valid && response.email) {
          ...
          setActivationStatus("invalid");
          setErrorMessage(
            response.message || "Invalid or expired activation token"
          );
        }
      } catch (error: any) {
        setActivationStatus("invalid");
        setErrorMessage(
          error.message ||
            "Failed to verify activation token. Please try again."
        );
      }
    };
    ...
    try {
      ...
      setTimeout(() => {
        navigate("/", {
          state: {
            message:
              "Email verified successfully! Please log in. Note: Your account must be activated by an administrator before you can access the system.",
          },
        });
      }, 2000);
    } catch (error) {
      ...
      >
        <Card style={{ width: 500, textAlign: "center" }}>
          <Space direction="vertical" size="large" style={{ width: "100%" }}>
            ...
            <Text type="secondary">
              {errorMessage ||
                "The activation link you used is invalid or has expired. Please contact your administrator for a new activation link."}
            </Text>
            <Button
              onClick={() => navigate("/")}
              style={{
